...
int secondary(int x)
{
    ...
    printf("Integer array via pointer:\n");
    for (int i = 0; i < 3; i++) {
        printf("iarray2Ptr[%d] = %d (address: %p)\n", i, *(iarray2Ptr + i), (void*)(iarray2Ptr + i));
    }

    printf("Character array via pointer:\n");
    for (int i = 0; i < 3; i++) {
        printf("carray2Ptr[%d] = %c (address: %p)\n", i, *(carray2Ptr + i), (void*)(carray2Ptr + i));
    }

    int* p;
    printf("Uninitialized pointer p value: %p\n", (void*)p);
